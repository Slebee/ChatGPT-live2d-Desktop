...
export function unDraggable(_model: Live2DModel) {
  const model = _model as DraggableLive2DModel;
  model.removeListener('pointerdown', onPointerDown);
  model.removeListener('pointermove', onPointerMove);
  model.removeListener('pointerup', onPointerUp);
  model.removeListener('pointerupoutside', onPointerUp);
}
